...
isCompleteExist = os.path.exists(PATH_TO_COMPLETE)

if not isRoiExist:
  os.makedirs(PATH_TO_ROI)
  print("The ROI directory is created!")
...
for label in label_lists:
    ...
    # Create folders
    path_class = os.path.join(PATH_TO_IMAGES, label)
...
for image_path in IMAGE_PATHS:
    ...
    destination = os.path.join(PATH_TO_COMPLETE,filename)
    ...
    for i in range(min(max_boxes_to_draw, boxes.shape[0])):
        if scores is None or scores[i] > Threshold:
            ...
            # print("Box coordinates", boxes[i].astype(int))
